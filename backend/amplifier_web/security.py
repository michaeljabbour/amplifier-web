...
def validate_path(path: str | Path, allowed_root: Path) -> tuple[bool, str, Path | None]:
    """
...
def validate_session_cwd(cwd: str | Path | None) -> tuple[bool, str, Path | None]:
    ...
    try:
        ...
        # Check for denied patterns before resolution
        cwd_str = str(cwd)
        for pattern in DENIED_PATH_PATTERNS:
            if pattern in cwd_str:
                return (
                    False,
                    f"CWD contains denied pattern '{pattern}': {cwd_str}",
                    None
                )
        ...
        if not resolved_cwd.exists():
            return (
                False,
                f"CWD does not exist: {resolved_cwd}",
                None
            )
